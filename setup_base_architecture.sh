...
# Provisions and configures the OpenLearningAnalytics base architecture. 
# Basic steps are:
...
storage_account_id="/subscriptions/$subscription_id/resourceGroups/$OLA_RESOURCE_GROUP/providers/Microsoft.Storage/storageAccounts/$OLA_STORAGE_ACCOUNT"
...
echo "--> Creating resource group: $OLA_RESOURCE_GROUP"
az group create -l $location -n $OLA_RESOURCE_GROUP

# 2) Create a budget for the resource group with an alert
email=$(az ad signed-in-user show --query mail -o tsv)
echo "--> Creating budget ($OLA_BUDGET) with alerts configured to be sent to: $email"
...
request="https://management.azure.com/subscriptions/${subscription_id}/resourceGroups/${OLA_RESOURCE_GROUP}/providers/Microsoft.Consumption/budgets/${OLA_BUDGET}?api-version=2019-10-01"
body=$(cat <<EOF
{
  "properties": {
    "category": "Cost",
...
echo "--> Creating storage account: ${OLA_STORAGE_ACCOUNT}"
az storage account create --resource-group $OLA_RESOURCE_GROUP --name ${OLA_STORAGE_ACCOUNT} --location $location \
...
az storage container create --account-name $OLA_STORAGE_ACCOUNT --name synapse --auth-mode login
az storage container create --account-name $OLA_STORAGE_ACCOUNT --name stage1 --auth-mode login
az storage container create --account-name $OLA_STORAGE_ACCOUNT --name stage2 --auth-mode login
az storage container create --account-name $OLA_STORAGE_ACCOUNT --name stage3 --auth-mode login
az storage container create --account-name $OLA_STORAGE_ACCOUNT --name test-env --auth-mode login

# 4) Create Synapse workspace, configure firewall access, and create spark pool
echo "--> Creating Synapse Workspace: $OLA_SYNAPSE"
...
az synapse workspace create --name $OLA_SYNAPSE --resource-group $OLA_RESOURCE_GROUP \
  --storage-account $OLA_STORAGE_ACCOUNT --file-system synapse --location $location \
...
synapse_principal_id=$(az synapse workspace show --name $OLA_SYNAPSE --resource-group $OLA_RESOURCE_GROUP --query identity.principalId -o tsv)
...
az synapse workspace firewall-rule create --name allowAll --workspace-name $OLA_SYNAPSE --resource-group $OLA_RESOURCE_GROUP \
  --start-ip-address 0.0.0.0 --end-ip-address 255.255.255.255

echo "--> Creating spark pool."
az synapse spark pool create --name spark1 --workspace-name $OLA_SYNAPSE --resource-group $OLA_RESOURCE_GROUP \
...
echo "--> Creating data factory instance: ${OLA_DATA_FACTORY}"
...
# az datafactory factory create --name $OLA_DATA_FACTORY --resource-group $OLA_RESOURCE_GROUP --location $location
...
request="https://management.azure.com/subscriptions/${subscription_id}/resourceGroups/${OLA_RESOURCE_GROUP}/providers/Microsoft.DataFactory/factories/${OLA_DATA_FACTORY}?api-version=2018-06-01"
body=$(cat <<EOF
{
  "name":"${OLA_DATA_FACTORY}",
  "location":"${location}",
  "properties":{},
...
adf_id=$(az datafactory factory show --factory-name $OLA_DATA_FACTORY --resource-group $OLA_RESOURCE_GROUP --query identity.principalId -o tsv)
...
properties='{"type":"AzureBlobFS","typeProperties":{"url":"https://'"${OLA_STORAGE_ACCOUNT}"'.dfs.core.windows.net"}}'
az datafactory linked-service create --factory-name $OLA_DATA_FACTORY --properties $properties --name $OLA_STORAGE_ACCOUNT --resource-group $OLA_RESOURCE_GROUP

# 6) Create machine learning resources (storage, keyvault, app insights, ml workspace)
echo "--> Creating storage account for ML workspace: ${OLA_ML_STORAGE_ACCOUNT}"
az storage account create --resource-group $OLA_RESOURCE_GROUP --name ${OLA_ML_STORAGE_ACCOUNT} --location $location \
  --kind StorageV2 --sku Standard_LRS --access-tier Hot --default-action Allow

echo "--> Creating key vault: ${OLA_KEYVAULT}"
az keyvault create --name $OLA_KEYVAULT --resource-group $OLA_RESOURCE_GROUP --location $location
...
az keyvault set-policy -n $OLA_KEYVAULT --secret-permissions get --object-id $adf_id

echo "--> Creating app-insights: $OLA_APP_INSIGHTS"
az monitor app-insights component create --app $OLA_APP_INSIGHTS --resource-group $OLA_RESOURCE_GROUP --location $location

ml_storage_account_id="/subscriptions/$subscription_id/resourceGroups/$OLA_RESOURCE_GROUP/providers/Microsoft.Storage/storageAccounts/$OLA_ML_STORAGE_ACCOUNT"
keyvault_id="/subscriptions/$subscription_id/resourceGroups/$OLA_RESOURCE_GROUP/providers/Microsoft.KeyVault/vaults/$OLA_KEYVAULT"
app_insights_id="/subscriptions/$subscription_id/resourceGroups/$OLA_RESOURCE_GROUP/providers/microsoft.insights/components/$OLA_APP_INSIGHTS"
az ml workspace create --workspace-name $OLA_ML_WORKSPACE --resource-group $OLA_RESOURCE_GROUP --location $location \
...
if [ "$include_groups" == "true" ]; then
  ...
  az role assignment create --role "Owner" --assignee $global_admins --resource-group $OLA_RESOURCE_GROUP
  ...
  stage3_id="/subscriptions/$subscription_id/resourceGroups/$OLA_RESOURCE_GROUP/providers/Microsoft.Storage/storageAccounts/$OLA_STORAGE_ACCOUNT/blobServices/default/containers/stage3"
  az role assignment create --role "Storage Blob Data Contributor" --assignee $external_data_scientists --scope $stage3_id
  testdata_id="/subscriptions/$subscription_id/resourceGroups/$OLA_RESOURCE_GROUP/providers/Microsoft.Storage/storageAccounts/$OLA_STORAGE_ACCOUNT/blobServices/default/containers/test-env"
  ...
  synapse_container_id="/subscriptions/$subscription_id/resourceGroups/$OLA_RESOURCE_GROUP/providers/Microsoft.Storage/storageAccounts/$OLA_STORAGE_ACCOUNT/blobServices/default/containers/synapse"
...
workspace_url=$(az synapse workspace show --name $OLA_SYNAPSE --resource-group $OLA_RESOURCE_GROUP | jq -r '.connectivityEndpoints | .web')
echo "--> Setup complete."
